...
contract SwapPool is Ownable {
    ...
    function setPromotePool(address _pool) public onlyOwner {
        promotePool = _pool;
    }
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256, uint256){
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            (uint256 ksaAmount, uint256 tokenAmount) = pendingKsaAndToken(_pid, _user);
            return (ksaAmount, tokenAmount);
        } else {
    ...
    function pendingKsaAndToken(uint256 _pid, address _user) private view returns (uint256, uint256){
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (user.amount > 0) {
            ...
            if (block.number > pool.lastRewardBlock) {
                uint256 blockReward = getKsaBlockReward(pool.lastRewardBlock);
                uint256 ksaReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
                accKsaPerShare = accKsaPerShare.add(ksaReward.mul(1e12).div(pool.totalAmount));
                return (user.amount.mul(accKsaPerShare).div(1e12).sub(user.rewardDebt), userPending);
            }
            if (block.number == pool.lastRewardBlock) {
                return (user.amount.mul(accKsaPerShare).div(1e12).sub(user.rewardDebt), userPending);
            }
    ...
    function deposit(uint256 _pid, uint256 _amount) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            depositKsaAndToken(_pid, _amount, msg.sender);
        } else {
            depositKsa(_pid, _amount, msg.sender);
        }
    }

    function depositKsaAndToken(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        updatePool(_pid);
        if (user.amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            withdrawKsaAndToken(_pid, _amount, msg.sender);
        } else {
            withdrawKsa(_pid, _amount, msg.sender);
        }
    }

    function withdrawKsaAndToken(uint256 _pid, uint256 _amount, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        require(user.amount >= _amount, "withdrawKsaAndToken: not good");
        updatePool(_pid);
        uint256 pendingAmount = user.amount.mul(pool.accKsaPerShare).div(1e12).sub(user.rewardDebt);
        if (pendingAmount > 0) {
            safeKsaTransfer(_user, pendingAmount);
    ...
    function emergencyWithdraw(uint256 _pid) public notPause {
        PoolInfo storage pool = poolInfo[_pid];
        if (isMultLP(address(pool.lpToken))) {
            emergencyWithdrawKsaAndToken(_pid, msg.sender);
        } else {
            emergencyWithdrawKsa(_pid, msg.sender);
        }
    }

    function emergencyWithdrawKsaAndToken(uint256 _pid, address _user) private {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
    ...
    function safeKsaTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > ksaBal) {
            ...
            PromotePool(promotePool).deposit(_to, ksaBal.mul(10).div(100));
        } else {
            ksa.safeTransfer(_to, _amount);
            PromotePool(promotePool).deposit(_to, _amount.mul(10).div(100));
        }
    }
